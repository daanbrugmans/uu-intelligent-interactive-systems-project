...
Smooth, natural transition.""",
    ...
    def _generate_response(self, state: AgentState, tone: str) -> AgentState:
        ...
        if customer_speech:
            ...
            if (
                any(kw in speech_lower for kw in cash_keywords)
    ...
    def _build_natural_prompt(
        ...
        if state["state"].conversation_history:
            ...
            for i, turn in enumerate(state["state"].conversation_history):
                emotion_name = (
                    turn.emotion.emotion_type.name if turn.emotion else "UNKNOWN"
                )
    ...
    def _get_fallback_response(
        self, emotion_type: EmotionType, round_num: int, state: ConversationState = None
    ) -> str:
        """
    ...
    def _finalize(self, state: AgentState) -> AgentState:
        ...
        logger.info(
            f"📍 Round {round_num} complete | Speech: {speech_dur:.1f}s | Continue: {not should_end}"
        )
